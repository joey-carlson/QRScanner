...
### Added
- **Kit Bundle domain-logic unit tests** (`KitBundleModelsTest`)
  - 18 tests covering requirement-fulfillment counting, duplicate DSN detection, battery-slot allocation, available-slot filtering, and the user-facing progress / missing-component messages
  - Raises `KitBundleState` to 100% and `RequirementStatus` to ~91% line coverage (both were 0%); overall line coverage 20.3% → 22.5%
  - No defects found — the pure model logic is sound (contrast with `DsnValidator`/`OcrConfidenceManager`, which surfaced bugs)
